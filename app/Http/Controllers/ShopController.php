...
class ShopController extends Controller
{
	...
	public function sortBook(ShopRequest $request)
	{
		if ($request->filter === 'sale') {
			return response($this->_bookRepository->sortByOnSale()->paginate($request->per_page));
		} elseif ($request->filter === 'popular') {
			return response($this->_bookRepository->sortByPopularity()->paginate($request->per_page));
		} elseif ($request->filter === 'price') {
			return response($this->_bookRepository->sortByPrice($request->order)->paginate($request->per_page));
		}
	}
